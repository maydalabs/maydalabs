import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Privacy policy – Emayda",
  description:
    "High-level overview of how Emayda handles website analytics, form submissions, email lists, and client project data.",
};

export default function PrivacyPage() {

  return (
    <div className="page">
      <h1 className="page-title">Privacy policy</h1>
      <p className="page-intro">
        High-level overview of how Emayda handles data across the website,
        analytics, and client work.
      </p>
      <p className="page-note">
        This is a placeholder to avoid 404s. When you&apos;re ready, this will
        be replaced with a proper policy, potentially generated with your legal
        counsel or a standard template adapted to your stack.
      </p>
      <ul className="page-list">
        <li>Website analytics (GA4 and similar tools).</li>
        <li>Form submissions and contact details.</li>
        <li>Email list management.</li>
        <li>Client project data and access.</li>
      </ul>
    </div>
  );
}
