...
describe("public page audiences", () => {
  it("removes the portrait only from the homepage", () => {
    expect(readFileSync("app/[lang]/(site)/page.tsx", "utf8")).not.toMatch(/portrait|<Profile/);
    expect(readFileSync("app/[lang]/(site)/profile/page.tsx", "utf8")).toContain("mehmet-e-mayda-portrait.jpg");
  });

  it("keeps Profile focused on professional work and contact, not application mechanics", () => {
    const profile = readFileSync("app/[lang]/(site)/profile/page.tsx", "utf8");
  ...
  it("makes About a company page with direct project contact and no hiring process", () => {
    const about = readFileSync("app/[lang]/(site)/about/page.tsx", "utf8");
