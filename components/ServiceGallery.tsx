...
import { SERVICES, SERVICE_DISPLAY_ORDER } from "@/lib/services";
...
export function ServiceGallery({ locale, overview = false }: { locale: Locale; overview?: boolean }) {
  ...
  const orderedServices = SERVICE_DISPLAY_ORDER.map(id => SERVICES[locale].find(service => service.id === id)!);
  return <section id={overview ? "service-options" : "services"} className={`svc-gallery${overview ? " is-overview" : ""}`}>
    ...
    <div className="svc-grid">
      {orderedServices.map((service, index) => <article id={overview ? service.id : undefined} key={service.id} className={`svc-card svc-card-${service.id}`}>
